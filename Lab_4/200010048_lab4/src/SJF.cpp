...
struct SJF
{
    ...
    {
        return lhs->burst_times->front() > rhs->burst_times->front();
    }
};
...
{
    if(argc != 2){
        cout << "Usage: ./SJF <process_file_path>" << endl;
        return 0;
    }
    ...
    priority_queue<Process*, vector<Process*>, SJF> cpu_ready, io_ready;
    HardWare cpu, io;
    ...
    print_metrics(metrics, file_path+".sjf.csv");
