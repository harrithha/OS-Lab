...
int main()
{
  ...
  for (int i = 0; i < input_array.size(); i++)
  {
    ...
    int cpuBurstTime = burst_times[i];
    int waitingTime = turnAroundTime - cpuBurstTime;
    int totalBurstTime = sumBurstTime(input_array[i]);
    float penaltyRatio = (waitingTime + totalBurstTime) * 1.0 / (totalBurstTime);
    avgResTime += responseTime;
    avgTurnAroundTime += turnAroundTime;
    avgWaitingTime += waitingTime;
    avgPenaltyRatio += penaltyRatio;
    cout << "Process ID " << processId + 1 << " Response Time " << responseTime << " Waiting Time " << waitingTime << " Turn Around Time " << turnAroundTime << " Completion Time " << comepletionTime << " Arrival Time " << arrivalTime << " Cpu Burst Time " << cpuBurstTime << " Total Burst Time " << totalBurstTime << " Penalty Ratio " << penaltyRatio << endl;
  }
  cout << "\n\n\n";
